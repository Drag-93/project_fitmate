...
public class    FileHandler {
    ...
    throws IOException {
        ...
        switch (tableType){
            ...
            case PRODUCT -> optionalFileEntity = fileRepository.findByProductEntity(
                    productRepository.findById(id).orElseThrow(()->new NoSuchElementException("존재하지않는 상품입니다."))
            );
...
//            File deleteFile = new File(fileUri);
    ...
    throws IOException{
        ...
        switch (tableType){
            ...
            case PRODUCT -> {
                ...
                optionalFileEntity = fileRepository.findByProductEntity(productEntity);
                //새 파일엔티티 생성
                fileEntity = FileEntity.builder()
                        .oldFileName(oldFileName)
                        .newFileName(newFileName)
    ...
    public void insertFile(String filePath, TableType tableType, Long id, MultipartFile file, ImageType imageType, int sortOrder)
            throws IOException{
        if (file == null || file.isEmpty()) {
            return; // 파일이 없으면 진행하지 않음
        }
