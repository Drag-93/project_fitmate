...
const TrainerReservation = () => {
  ...
  const changeStatus = async (id, status) => {
    ...
    try {
      ...
      alert(`예약이 ${status === "COMPLETE" ? "완료" : "취소"} 처리되었습니다.`);
      getReservationList();
    } catch (error) {
      console.error(error);
  ...
  const stats = {
    ...
    reserved: reservations.filter((r) => r.reservationStatus === "RESERVED").length,
    complete: reservations.filter((r) => r.reservationStatus === "COMPLETE").length,
  ...
  return (
    <div className="trainer-reservation-container">
      ...
      <div className="page-header">
        <h2>PT 수업 예약 관리</h2>
        <p className="subtitle">회원님들이 신청한 PT 예약 및 수업 일정을 관리합니다.</p>
      </div>
            ...
            <div key={reservation.id} className="reservation-card">
              <div className="card-header">
                <div className="member-info">
                  ...
                  <h3 className="member-name">{reservation.memberName} 회원님</h3>
                </div>
